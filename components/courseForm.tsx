  ...
  async function submit() {
    try {
      ...
      const path = await axios.post("/api/upload",fileBody);
       address = path.data
       data = { name, description, address, link, price };
      } else {
        data = {name, description, url, link, price}
      }
      let result;
      if(_id){
         result = await axios.put(`/api/courses?id=${_id}`, {...data})
      } else {
         result = await axios.post("/api/courses", {...data})
      }
      if (result) {
